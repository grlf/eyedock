...
function modChrome_VertexCalc($module, &$params, &$attribs)
{
	$badge          = preg_match ('/badge/', $params->get('moduleclass_sfx'))? '<span class="badge">&nbsp;</span>' : '';
	$moduleTag      = htmlspecialchars($params->get('module_tag', 'div'));
	$headerTag      = htmlspecialchars($params->get('header_tag', 'h3'));
	...
	if (!empty ($module->content)) {
		...
		if ($module->showtitle != 0) {
			$html .= "<{$headerTag} class=\"module-title {$headerClass}\"><a href=\"#\" onclick=\"playVid('vidPane');\" title=\"Quick Calcs\"><span>{$module->title}</span></a></{$headerTag}>";
		}
