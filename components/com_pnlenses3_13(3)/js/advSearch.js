...
//hides the "+" button on all but the last row. hides the "x" button if only one row
/* 
function hideShowCloneRemove (){
	jQuery(".removeFilterRow_btn").show();
	var rowCount = jQuery(".filterRow").length;
	if (rowCount ==1) jQuery(".removeFilterRow_btn").hide();
}
*/
...
function makeSelect (valObj) {
	...
	var select = "<select class='operatorVal' >";
	for (var i = 0; i < list.length; i++) {
...
function populateSearchForm (params) {
	...
	if (optionsPopulated == 0) return; //don't try to populate the search form before the select boxes are populated!
	...
	for (var prop in params){
		if (jQuery.inArray(prop, searchOptionIDs) > -1  ) {
			...
			for (var item in uniqueProps) { 
				...
				//var operatorLength = jQuery(operatorSelect).length;		
				if (operatorSelect && operatorSelect.length) jQuery(operatorSelect).val( (pVal.indexOf("-") == -1 )?"+":"-" );
				var rhs = jQuery(el).parent().nextAll(".filterRHS_div").find("select");
				//var rhsLength = jQuery(rhs).length;	
				//remove the first "+" or "-" from the propVal if there was an operator select
				if (operatorSelect && operatorSelect.length) pVal = pVal.substr(1);
				if (rhs && rhs.length) jQuery(rhs).val(pVal);
			}
		}
	}
