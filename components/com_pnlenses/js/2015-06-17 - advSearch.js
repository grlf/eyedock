...
function clearReplaceButtons() {
	...
	jQuery('#adv-search-replace input').attr('checked', false).button().button('refresh');
	jQuery('#adv-search-replace #replace0').attr('checked', true).button().button('refresh');
...
function setParamsRadios (item, value) {
	...
	jQuery("#adv-search-" + item + "-radio radio").attr('checked', false).button().button('refresh');
    try {
        if (value == 1) jQuery("#" + item + "-yes").attr('checked', true).button().button('refresh');
        if (value == 0) jQuery("#" + item + "-no").attr('checked', true).button().button('refresh');
        if (value == -1) jQuery("#" + item + "-any").attr('checked', true).button().button('refresh');
    } catch (e) { }
}
