...
function populateSearchForm (params) {
	...
	//get the following radiobuttons properly checked
	...
	for (var prop in params){
		if (jQuery.inArray(prop, searchOptionIDs) > -1  ) {
			var propVal = jQuery.bbq.getState(prop); // get the prop's value
			for (var item in propVal) { //the propVal will be an array, eg diameter[]=14.0
				...
				if (operatorSelect) jQuery(operatorSelect).val( (pVal.indexOf("-") == -1 )?"+":"-" );
				...
				if (operatorSelect) pVal = pVal.substr(1);
				if (rhs) jQuery(rhs).val(pVal);
			}
		}
	}
