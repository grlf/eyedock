...
class PnlensesModelPnlenses extends JModel {
  ...
  function getLenses(){
    ...
    function getLensesFromArray($arr) {
		...
		$query = "SELECT d.pn_tid, d.pn_name as name, d.pn_image as image, pnlc.pn_comp_name as company,	d.pn_max_plus, d.pn_max_minus, d.pn_max_diam, d.pn_min_diam, d.pn_diam_all, d.pn_bc_all, d.pn_max_cyl_power, d.pn_max_add, d.pn_cosmetic, d.pn_dk, d.pn_oz, d.pn_ct,pnlp.pn_h2o, d.pn_discontinued as discontinued, pop.score,COALESCE(c.`favorite`, 0) favorite FROM (SELECT a.pn_tid, a.pn_comp_id, a.pn_poly_id, a.pn_name, a.pn_image, b.user_id,a.pn_max_plus, a.pn_max_minus, a.pn_max_diam, a.pn_min_diam, a.pn_diam_all, a.pn_bc_all, a.pn_max_cyl_power, a.pn_max_add, a.pn_cosmetic, a.pn_dk, a.pn_oz, a.pn_ct, a.pn_discontinued FROM pn_lenses a CROSS JOIN	(SELECT DISTINCT user_id FROM pn_lenses_user_favorites) b) d LEFT JOIN pn_lenses_user_favorites c ON c.item_id = d.pn_tid AND c.user_id = d.user_id left join pn_lenses_companies pnlc on (d.pn_comp_id = pnlc.pn_comp_tid) left join pn_lenses_polymers pnlp on (d.pn_poly_id = pnlp.pn_poly_tid) left join pn_lenses_popularity pop on (pop.itemID = d.pn_tid) WHERE d.user_id = $user->id AND d.pn_tid IN  (" . implode(',',$arr) . ") ORDER BY favorite DESC, score DESC, name ASC";		
    ...
    function findCLsByPower ($params) {
    	...
    	if (!$refraction) $error_msg[] = "A refraction is needed!";
    	if ($bifocal == 1) $error_msg[] = "Sorry - Bifocal lenses cannot be searched by refraction. Only single vision lenses were returned.";
    	$params['bifocal'] = 0;
		...
		if( !isset($toric) ) {  //not specified if toric or spherical
