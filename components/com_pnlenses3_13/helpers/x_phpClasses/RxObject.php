...
class RxObject 
{
  ...
  { 
	...
	public function setCyl($x) { 
 	...
 	public function isPlusCyl() { return $this->plusCyl; } 
	public function isMinusCyl() { return $this->minusCyl; } 
	...
	public function diffVertex ($vertexTo = 0) {
		...
		$newSph = 1/( 1/ $oldSph - $vertexChange );
		$newCyl = 1/( 1/ $oldCyl - $vertexChange );
	...
	public function isWTR () {
		...
		return ( $this->axisP() <= 30 || $this->axisP() >= 150)?1:0;
	}
	
	public function isATR () {
		return ( $this->axisP() >= 60 && $this->axisP() <= 120)?1:0;
	}
