...
class PnlensesModelPnlenses extends JModel {
  ...
  function getLenses(){
    ...
    function suggestLensPowersForLens() {
        	...
        	$returnLenses = array();
        	$resultArray = getBestCLPowerForParams ($refraction, $lensPowerArr, $lensPowerArr[0]['toric'])  ;
...
FROM pn_lenses d
LEFT JOIN pn_lenses_user_favorites c ON c.item_id = d.pn_tid
...
WHERE f.item_id = d.pn_tid
AND f.user_id =$user->id
AND f.favorite=1
)
AND
d.pn_tid IN (" . implode(',',$arr) . ") ) 
...
//         $data['lenses'] = $lenses;
    ...
    function getLensPowerListsFromArray($arr){
    	...
    	$query = "
(SELECT pnl.pn_tid, pnl.pn_name as name, pnl.pn_image as image, pnlc.pn_comp_name as company, pnl.pn_cosmetic, pnl.pn_dk, pop.score, pnl.pn_discontinued as discontinued,  1 AS favorite
FROM pn_lenses pnl 
LEFT JOIN pn_lenses_companies pnlc ON (pnl.pn_comp_id = pnlc.pn_comp_tid) 
LEFT JOIN pn_lenses_polymers pnlp ON (pnl.pn_poly_id = pnlp.pn_poly_tid) 
...
AND pn_bifocal = 0)
UNION DISTINCT
(SELECT pnl.pn_tid, pnl.pn_name as name, pnl.pn_image as image, pnlc.pn_comp_name as company, pnl.pn_cosmetic, pnl.pn_dk, pop.score, pnl.pn_discontinued as discontinued, 0 as favorite
FROM pn_lenses pnl 
LEFT JOIN pn_lenses_companies pnlc ON (pnl.pn_comp_id = pnlc.pn_comp_tid) 
LEFT JOIN pn_lenses_polymers pnlp ON (pnl.pn_poly_id = pnlp.pn_poly_tid) 
LEFT JOIN pn_lenses_popularity pop ON (pop.itemID = pnl.pn_tid) 
...
function removeVAOutliers($lensesArray) {
	...
	//compare each lens's VA to the best VA. If it's less more than 25 points away, don't include it
	foreach ($lensesArray as $lens){
		$va = $lens['va'];
		//echo "<p>$va</p>";
		if (abs($va - $bestVA) <25) array_push($returnArray, $lens);
	}
