...
// 		echo "</p>";
	...
	if (isset($sph) && abs($sph) > .25)  $where[]= makeWhereChunk ("pn_max_plus", $sph, "AND", "<>", "pn_max_minus", $tolerance);
 ...
 */
 ...
 $query = "(SELECT d.pn_tid, d.pn_name as name, d.pn_image as image, pnlc.pn_comp_name as company, d.pn_cosmetic,  d.pn_toric as toric, d.pn_dk, pop.score, d.pn_discontinued as discontinued,  1 AS favorite
FROM pn_lenses d
LEFT JOIN pn_lenses_companies pnlc ON ( d.pn_comp_id = pnlc.pn_comp_tid ) 
LEFT JOIN pn_lenses_polymers pnlp ON ( d.pn_poly_id = pnlp.pn_poly_tid ) 
LEFT JOIN pn_lenses_popularity pop ON ( pop.itemID = d.pn_tid ) 
...
SELECT d.pn_tid, d.pn_name as name, d.pn_image as image, pnlc.pn_comp_name as company, d.pn_cosmetic,  d.pn_toric as toric, d.pn_dk, pop.score, d.pn_discontinued as discontinued,  0 AS favorite
FROM pn_lenses d
LEFT JOIN pn_lenses_companies pnlc ON ( d.pn_comp_id = pnlc.pn_comp_tid ) 
LEFT JOIN pn_lenses_polymers pnlp ON ( d.pn_poly_id = pnlp.pn_poly_tid ) 
LEFT JOIN pn_lenses_popularity pop ON ( pop.itemID = d.pn_tid ) 
...
function removeVAOutliers($lensesArray) {
	...
	$threshold = 15;  //how big of a difference in VA will be acceptable (eg 20/40 vs 20/20 = 20)
	...
	foreach ($lensesArray as $lens){
		$va = $lens['va'];
		//echo "<p>$va</p>";
		if (abs($va - $bestVA) <15) array_push($returnArray, $lens);
	}
