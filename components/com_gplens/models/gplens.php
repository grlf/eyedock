...
class GpLensModelGpLens extends Jmodel {
	...
	const LENSIMGURL = '/administrator/components/com_gplens/models/images/';
	const LENSPDFURL = '/administrator/components/com_gplens/models/pdfs/';
	...
	function getLensesBySearchstring($string) {
		...
		foreach($terms as $t) {	
			$ts[] = 'l.name REGEXP "[[:<:]]' . $t . '[[:>:]]"';
			$ts[] = 'l.aliases REGEXP "[[:<:]]' . $t . '[[:>:]]"';
		}
		$finalterms = implode(' OR ', $ts);
		*/
		
		foreach($terms as $t) {
			$nameterms[] = 'l.name LIKE "%' . $t . '%"';
			$aliasterms[] = 'l.aliases LIKE "%' . $t . '%"';
		}
		
		$finalnameterms = implode(' AND ', $nameterms);
		$finalaliasterms = implode(' AND ', $aliasterms);
		$finalterms = '(' . $finalnameterms . ') OR (' . $finalaliasterms . ')';
		...
		left join rgpDesignCategory dc on (l.designCategoryID = dc.tid) 
		left join rgpSubcategory sc on (l.subcategoryID = sc.tid) 
		where ' . $finalterms . ' order by lens_name';
