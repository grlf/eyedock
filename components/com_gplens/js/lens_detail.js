var Details = new Class({
	...
	initialize: function(container, joomla_shell, left_box, right_box, options) {
		...
		this.wrapper_starting_height = this.wrapper.getStyle('height');
	...
	show_and_gloss_lens_details: function(html) {
		...
		this.lens_details.inject(this.joomla_shell);
		
		var wrapper_coords = this.wrapper.getCoordinates();
		var details_coords = this.lens_details.getCoordinates();
		
		if(details_coords.bottom > wrapper_coords.bottom) {
			this.wrapper.setStyle('height', wrapper_coords.bottom);
		}
	...
	//	console.log(materials_json);
		...
		if(lens_json.c_url != undefined) {
			if(lens_json.c_url.substring(0,4) != 'http' && lens_json.c_url != '') { lens_json.c_url = 'http://' + lens_json.c_url; }
		}
		
		var company_tooltip_content = '';
		company_tooltip_content = company_tooltip_content + '<div class="gp_tooltip_closer"><a href="#">[X]</a></div>';
		if(lens_json.company != '' && lens_json.company != undefined) { company_tooltip_content = company_tooltip_content + '<div class="gp_tooltip_lens_wcloser">' + lens_json.company + '</div>'; }
		...
		if(lens_json.c_url != '' && lens_json.c_url != undefined) { company_tooltip_content = company_tooltip_content + '<div class="gp_tooltip_type"><div class="gp_tooltip_label">Website:</div> <a href="' + lens_json.c_url + '" target="_blank">' + lens_json.c_url + '</a></div>'; }
			...
			'mousedown': function(e) {
				...
				if(company_tooltip_content != '') {
					...
					if(this.is_company_tooltip == false) {
						...
						this.company_tooltip.getFirst('div').addEvent('mousedown', function() {
				...
				'mousedown': function(e) {
					...
					if(this.is_materials_tooltip == false) {
						...
						this.materials_tooltip = new Element('div', {
							html: '<div class="gp_tooltip_closer"><a href="#">[X]</a></div>',
							'class': 'gp_tooltip_eyedock',
							styles: {
						...
						this.materials_tooltip.getFirst('div').addEvent('mousedown', function() {
	...
	close_lens_details: function() {
		...
		this.wrapper.setStyle('height', this.wrapper_starting_height);
	...
	show_and_gloss_material_details: function() {
		...
		var container_coords = this.container.getCoordinates();
		this.material_details.inject(this.joomla_shell);
		...
		if(material_json.c_url != undefined) {		
			if(material_json.c_url.substring(0,4) != 'http' && material_json.c_url != '') { material_json.c_url = 'http://' + material_json.c_url; }
		}
		
		var company_tooltip_content = '';
		company_tooltip_content = company_tooltip_content + '<div class="gp_tooltip_closer"><a href="#">[X]</a></div>';		
		company_tooltip_content = company_tooltip_content + '<div class="gp_tooltip_lens_wcloser">' + material_json.company + '</div>';
		...
		if(material_json.c_url != '') { company_tooltip_content = company_tooltip_content + '<div class="gp_tooltip_type"><div class="gp_tooltip_label">Website:</div> <a href="' + material_json.c_url + '" target="_blank">' + material_json.c_url + '</a></div>'; }
