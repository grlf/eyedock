var RightBox = new Class({
	...
	populate_lens_list_by_company_id: function(lenses) {
		...
		lenses.each(function(l) {
			...
			lens.store('id', l.tid);
			if(l.lens_name != null) { lens.store('lens_name', l.lens_name); } else { lens.store('lens_name', '?'); }
			if(l.company != null) { lens.store('company', l.company); } else { lens.store('company', '?'); }
			if(l.type != null) { lens.store('type', l.type); } else { lens.store('type', '?'); }
			if(l.subtype != null) { lens.store('subtype', l.subtype); } else { lens.store('subtype', '?'); }
