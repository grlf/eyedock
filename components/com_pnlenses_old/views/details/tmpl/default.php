...
<div class="edas-ld-content">
	<div class="edas-ld-titleheader <? if ($this->lens['discontinued'] == 1) echo 'detail-discontinued'; ?>">
		<div class="ed-ld-lenstitle"><?=$this->lens['name']?>	
		
		<?  //subscriber only
			if ($subscriber == 1) { ?>
			
		<div class="ed-ld-stardiv">	<a class="rating<? if ($this->lens['favorite']==1) echo ' on'; ?>" href="#1" title="favorite" id="display-star<?=$this->lens['id']?>" onClick="favStarClick(event, <? echo $this->lens['id'] . ", " . $user->id ?>);">Star</a> </div>
		...
		<div class="ed-ld-closediv">
		<!--<input class="ed-ld-close-btn" type='button' onclick="do_close_lens();" value="<--" > -->
			<img src="/components/com_pnlenses/images/left_arrow.png" class="ed-ld-close-btn" onclick="closeDetails()"/>
		
		</div>
			<img src="/components/com_pnlenses/images/printer.png" class="ed-ld-print_btn"  onclick="printLensDetails()"/>

		<!--<input class="ed-ld-print_btn" type='button' onclick="printLensDetails() ;" value="print" >-->
...
<?
              ...
              if($this->lens['bifocal'] != 1 && strpos($this->lens['name'], "Synergeye") == false && strpos($this->lens['name'], "Softperm") == false && $subscriber == 1) { ?>
							<div class="ed-ld-mrsearch">
								...
								<!--onkeydown="findTrialLens(event, <?=$this->lens['id']?>);-->
								<img id="ed-suggest-loader" src="http://www.eyedock.com/components/com_pnlenses/images/cl_spinner.gif" alt="loading" /> 
							</div>
							
							<div id="suggestTrialLensResults">
								<div id="ed-details-suggest-cross"></div>
								<div id="ed-details-suggest-lenses"></div>
							</div>

<?
              }

              if(trim($this->lens['prices']) != '' && trim($this->lens['prices']) != '0.00'  && $subscriber == 1){
              ?>
							<div class="edas-ld-wholesale-view">
								<a href="javascript:;" onclick="detail_wholesale_toggle()">View Wholesale Prices</a> 
							</div>
							<div id="edas-ld-wholesale"  style="display: none;">
								<div>
...
<?=$this->lens['company_email']?>
						...
						<p><a href='/index.php?option=com_pnlenses&comp_id=<?= $this->lens['comp_id'] ?>'>View all <?=$this->lens['company'] ?> lenses</a><p>
					</div>
					
					<div class="edas-ld-detailfooter">
						<g:plusone size="small" href="http://www.eyedock.com/index.php?option=com_pnlenses&Itemid=83&lens_id=<?= $this->lens['id']; ?>"></g:plusone>
					</div>
