...
class GpMaterialModelGpMaterial extends Jmodel {
	...
	function getCompanies() {
		...
		$query = 'SELECT tid, name from rgpMaterialCompany ORDER BY name';
	...
	function getMaterialsByCompanyId($ids) {
	
		$mysqli = new mysqli(self::GPHOST, self::GPUSER, self::GPPASS, self::GPDB);
		
		foreach($ids as $id) {
			$where[] = 'm.rgpMaterialCompanyID = ' . $id;
		}
		
		$where = implode(' OR ', $where);
		
		$query = 'select m.tid as id, m.name as material_name, m.dk as dk, m.wetAngle as wetangle, m.specificGravity as specificgravity, m.refractiveIndex as refractiveindex, mt.name as materialtype, mc.name as company 
		from 
		rgpMaterials m 
		left join rgpMaterialType mt on (m.materialTypeID = mt.tid) 
		left join rgpMaterialCompany mc on (m.rgpMaterialCompanyID = mc.tid) 
		where ' . $where . ' order by material_name asc';
	...
	function getMaterialsBySearchstring($string) {
		...
		$finalterms = implode(' OR ', $ts);
		*/
		
		foreach($terms as $t) {
			$nameterms[] = 'm.name LIKE "%' . $t . '%"';
		}
		
		$finalterms = implode(' AND ', $nameterms);
		...
		left join rgpMaterialType mt on (m.materialTypeID = mt.tid) 
		left join rgpMaterialCompany mc on (m.rgpMaterialCompanyID = mc.tid) 
		where ' . $finalterms . ' order by m.name asc';
	...
	function getMaterialsBySliders($data) {
	
		$mysqli = new mysqli(self::GPHOST, self::GPUSER, self::GPPASS, self::GPDB);
		
		$dk_cond = ($data->dk->gl == 'greater') ? '(m.dk >= ' . $data->dk->val . ')' : '(m.dk <= ' . $data->dk->val . ')';
		$wetting_cond = ($data->wetting->gl == 'greater') ? '(m.wetAngle >= ' . $data->wetting->val . ')' : '(m.wetAngle <= ' . $data->wetting->val . ')';
		$refractive_cond = ($data->refractive->gl == 'greater') ? '(m.refractiveIndex >= ' . $data->refractive->val . ')' : '(m.refractiveIndex <= ' . $data->refractive->val . ')';
		$specific_cond = ($data->specific->gl == 'greater') ? '(m.specificGravity >= ' . $data->specific->val . ')' : '(m.specificGravity <= ' . $data->specific->val . ')';
		
		$cond = $dk_cond . ' and ' . $wetting_cond . ' and ' . $refractive_cond . ' and ' . $specific_cond;
	...
	/*	$dk_tol = 10;
		...
		$welting_ceil = $welting + $welting_tol;
		$welting_floor = $welting - $welting_tol;
		
		$refractive_ceil = $refractive + $refractive_tol;
		$refractive_floor = $refractive - $refractive_tol;

		$specific_ceil = $specific + $specific_tol;
		$specific_floor = $specific - $specific_tol;
	*/	
		...
		left join rgpMaterialType mt on (m.materialTypeID = mt.tid) 
		left join rgpMaterialCompany mc on (m.rgpMaterialCompanyID = mc.tid) 
		where ' . $cond . ' order by material_name asc';
							...
							<div id="gp_right_box_lens">
								...
								<div id="gp_right_box_content_lens">
									...
									<div id="gp_lens_package_insert" class="gp_details_section">
										<?php if($lens->pdf != '') { ?><a class="gp_details_link" href="<?php echo self::LENSPDFURL . $lens->pdf; ?>">Link to Lens's Package Insert (.pdf)</a><?php } ?>
									</div>
