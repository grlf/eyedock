  ...
  function getLenses(){
	...
	//print_r($params);
		...
		if (isset($clRx[0])) {
			...
			if(!isset($toric) ) $toric = $lensRxVals['toric'];
	...
	if ( isset($toric) && $toric == 0) $where[] = "pn_toric = 0 ";
	...
	if ($toric == 1 || ( (isset($axisSteps) || isset($oblique) || isset($cyl)))  ) {
		$toric = 1;
		$where[]= "   pn_toric = 1 " ;
    ...
    function getLensesFromArray($arr) {
		$query = "select pnl.pn_tid, pnl.pn_name as name, pnl.pn_image as image, pnlc.pn_comp_name as company, 
		pnl.pn_max_plus, pnl.pn_max_minus, pnl.pn_max_diam, pnl.pn_min_diam, pnl.pn_diam_all, pnl.pn_bc_all, pnl.pn_max_cyl_power, pnl.pn_max_add, pnl.pn_cosmetic, pnl.pn_dk, pnl.pn_oz, pnl.pn_ct, pnlp.pn_h2o, ufav.favorite, pop.score,
		pnl.pn_discontinued as discontinued 
		from 
		pn_lenses pnl 
		left join pn_lenses_companies pnlc on (pnl.pn_comp_id = pnlc.pn_comp_tid)
