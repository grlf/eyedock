...
function updateRHS (obj){
	if (obj.input == "bool") return "<select class='rhsVal' onChange='submitSearch()' ><option value=''>any</option><option value='1'>yes</option><option value='0'>no</option></select>";
...
function submitSearch (){
	...
	jQuery('#ed-lenses-paramsList').fadeOut();
	
	jQuery.ajax({
        type: "POST",
        url: componentURL + "&view=paramlist&format=raw&task=paramSearch" + layout,
        data: data,
        dataType: "html",
        success: function(result){
            //var result = jQuery(xml).find("result");
            jQuery('#ed-lenses-paramsList').html(result);
             jQuery('#ed-lenses-paramsList').fadeIn();
            jQuery('#adv-searches-loader').hide();
        }
    });
	

    jQuery('#ed-lenses-listHeader' ).show();
}
