...
class Am_Form_Element_Category extends HTML_QuickForm2_Container_Group {
    public function __construct($name = null, $attributes = null, $data = null)
    {
        ...
        $sel = $this->addSelect($name, array('multiple'=>'multiple', 'class'=>'magicselect'))
            ->loadOptions($data['options']);
        $id = $sel->getId();

        $this->addHtml()->setHtml(sprintf('<div><a class="local" id="edit-%s-link" href="%s" target="_top">%s</a></div>',
            $id, Am_Controller::escape(REL_ROOT_URL . '/' . $data['base_url']),
            $data['link_title']));
        $root = REL_ROOT_URL;
        $this->addScript()
            ->setScript(<<<CUT
$('#edit-$id-link').click(function(){
    var div = $('<div></div>');
    $('body').append(div);
    div.load(this.href, function(){
        div.dialog({
            ...
            modal: true,
            autoOpen : true,
            width: Math.min(700, Math.round($(window).width() * 0.7)),
            buttons: {
            ...
            close : function(){
                div.remove();
                $.get('$root/{$data['base_url']}/options', function(res){
                    $('#$id').empty();
                    for (var k in res) {
                        $('#$id').append($('<option></option>').attr('value', k).html(res[k]));
                    }
                    $('#$id').restoreMagicSelect();
                })
            }
        })
    })
...
class Am_Form_Element_Upload extends HTML_QuickForm2_Element_InputText {
    ...
    public function  __construct($name = null, $attributes = null, $data = null) {
        ...
        $attributes['data-info'] = Am_Controller::getJson(array());
        $attributes['data-error'] = Am_Controller::getJson(array());
...
class Am_Form_Element_EmailLink extends HTML_QuickForm2_Element {
    ...
    public static function getEditLink($el, $day = null, $product_id=null) {
        ...
        return sprintf('<a href="%s" target="_top" class="email-template local" rel="%s">%s</a>',
