...
abstract class Am_Grid_Editable_Content extends Am_Grid_Editable
{
        ...
        try{
            ...
            if (!$upload)
                return $this->renderTd(
                    ...
                    ' <span style="color: grey">(' . ___('Error from Storage Engine') . ': ' . Am_Controller::escape($e->getMessage()) . ')</span>' .
                    '<br />' . ___('Real Path') . ': ' . Am_Controller::escape($file->path), false);
        }
    ...
    protected function getTemplateOptions()
    {
        $folders = array(
            ...
            APPLICATION_PATH . '/default/themes/' . $this->getDi()->config->get('theme') => 2,
        );
        $ret = array(
            null => 'layout.phtml'
        );
    ...
    {
        $category = $form->addCategory('_category', null, array(
