...
class Am_Paysystem_EwaySp extends Am_Paysystem_Abstract
{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        $form->addText('CustomerID', array('size' => 20))
            ->setLabel(array('Customer ID', 'Your eWAY Customer ID'))
            ->addRule('required');

        $form->addText('UserName', array('size' => 20))
            ->setLabel(array('User name', 'Your eWAY Customer User Name'))
            ->addRule('required');

        $form->addText('PageTitle ', array('size' => 60))
            ->setLabel(array(
                'Page Title',
                'This is value will be displayed as the title of the browser.
                Default: eWAY Hosted Payment Page'));

        $form->addText('PageDescription', array('size' => 200))
            ->setLabel(array(
                'Page Description',
                'This value will be displayed above the Transaction Details.
                Default: Blank'));

        $form->addText('PageFooter', array('size' => 60))
            ->setLabel(array(
                'Page Footer',
                'This value will be displayed below the Transaction Details.'));

        $form->addText('CompanyLogo', array('size' => 200))
            ->setLabel(array(
                'URL company logo',
        ...
        $form->addText('Pagebanner', array('size' => 200))
            ->setLabel(array(
                'URL page banner',
                'The url of the image can be hosted on the
                merchants website and pass the secure
        ...
        $form->addText('CompanyName', array('size' => 60))
            ->setLabel(array(
                'Company name',
