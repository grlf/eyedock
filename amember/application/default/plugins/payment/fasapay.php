...
class Am_Paysystem_Fasapay extends Am_Paysystem_Abstract{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        $form->addText('fp_account')
            ->setLabel(array('Your FasaPay Store Account'))
            ->addRule('required');

        $form->addText('fp_store')
            ->setLabel(array('FasaPay Store Name'))
            ->addRule('required');

        $form->addText('security_word', array('size' => 40))
            ->setLabel(array('FasaPay Store Security Word'))
            ->addRule('required');

        $form->addAdvCheckbox('is_sandbox')
            ->setLabel(array('Sandbox Mode Enabled', "use sandbox account data from http://sandbox.fasapay.com/"));
    }
