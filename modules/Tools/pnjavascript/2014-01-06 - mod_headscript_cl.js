...
function displayAnswer(thoughts)
{
   ...
   if (Search.msg!="")
   {
      ...
      answer += "<input type='hidden'  name='and[]' value='replace_simple' />";
      answer += "<p>Replacement<br/> at least: <select name='replace_simple[0]' id='replace_simple[0]' onChange='changed();'><option value='>=0' selected>one day</option><option value='>=6'>one week</option><option value='>=12'>two weeks</option><option value='>=27'>monthly</option><option value='>=85'>3 months</option><option value='>=170'>6 months</option><option value='>=350'>yearly</option></select>";
      answer += "<br/>at most:<select name='replace_simple[1]' id='replace_simple[1]' onChange='changed();'><option value='<=2'>one day</option><option value='<=8'>one week</option><option value='<=16'>two weeks</option><option value='<=35'>monthly</option><option value='<=95'>3 months</option><option value='<=190'>6 months</option><option value='<=400' selected>yearly</option></select></p>";


      if (Search.thin==1) answer+="<br/><input name='search[toric_type_simple]' type='checkbox' value=' LIKE \'%thin%\'' >thin zones ";


      answer+="<input type='hidden' id='display_phrase' name='display_phrase' value=' that are available in this power: "+ Lens.power+".' >"
      
      answer+="<br/><input type='submit' value='find lenses' >"
...
function softSph()
{
   ...
   if (MR.refractiveCyl()>.6)
   {
      ...
      if (percentCyl<.25) thoughts+="<br/><br/>...Then again, because the cylinder is such a small percentage of the overall prescription, they may not be very sensitive to it.";
...
if (Math.abs(MR.sphEquiv())>9)
   {
      Search.msg=" Your options are more limited with this magnitude of power.  Fill out this form to see lenses that are available in this range.";

   } else {
      Search.msg="  You shouldn't need much help finding lenses in this power range but if you want some, fill out this form."
   }
     
     answer =  "<input type='hidden' id='search[toric]' name='search[toric]' value='=0'>";
     
     if ( MR.sphEquiv()<0){
        answer += "<input type='hidden' name='search[max_minus]' id='search[max_minus]' value='<="+ signNum(rounder(MR.sphEquiv()))+"'>";
     } else {
        answer += "<input type='hidden' name='search[max_plus]' id='search[max_plus]' value='>="+ signNum(rounder(MR.sphEquiv()))+"'>";
     }
...
function softToric()
{
   ...
   if (Math.abs(MR.sph)>6 || Math.abs(MR.cyl)>2.25)
   {
     ...
     Search.msg+="licking the button below will help you find some options."

      answer =  "<input type='hidden' id='search[toric]' name='search[toric]' value='=1'>";

      most_oblique = MR.axis;
      if (most_oblique > 90) most_oblique = 180 - most_oblique;
      if (most_oblique > 45) most_oblique = 90 - most_oblique;

      answer += "<input type='hidden' name='search[oblique]' id='search[oblique]' value='>="+ most_oblique+"'>";
      
      answer +="<input type='hidden' name='search[max_cyl_power]' id='search[max_cyl_power]' value='<="+MR.minusCylcyl()+"'>";
...
function RGPbackToric()
{
   ...
   if (Cornea.cylType()=="ATR")
   {
      if (Cornea.cornealCyl()>=MR.refractiveCyl() )
      {
         ...
         thoughts+="Back toric lenses work their best when the corneal cylinder is greater than the refractive cylinder, which is not the case in this patient.";
