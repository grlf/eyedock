...
class RxpreserveViewPreservatives extends JView {
	...
	//	}
		...
		$query = 'SELECT * from pn_rx_preserve';
	...
	//	echo $total;
		...
		$mysqli = new mysqli(self::GPHOST, self::GPUSER, self::GPPASS, self::GPDB);
		$query = 'SELECT * from pn_rx_preserve ' . $where . ' ORDER BY pn_name' . $limiter;
		$result = $mysqli->query($query);
		while($obj = $result->fetch_object()) {
			$preservatives[] = $obj;
		}
		
		$this->assignRef('preservatives', $preservatives);
