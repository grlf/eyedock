...
require_once( JPATH_ROOT.DS.'utilities/powerLists/makePowerLists.php' );
...
class PnlensesModelPnlenses extends JModel {
	...
	function addNewLens($data) {
		...
		if($result = $mysqli->selectQuery($query)) {
			...
			// add a row in the power row table for the new lens
			$lastID = $mysqli->last_ID();
			//$insert_id = mysqli_insert_id($mysqli->getConnection());
			//echo "last! " . $lastID;
			$pwrQuery = "INSERT INTO eyedock_data.pn_lenses_powers (id, lensID, variation, baseCurve, diameter, sphere, cylinder, axis, addPwr, colors_enh, colors_opq) VALUES (NULL, $lastID, NULL, '', '', '', NULL, NULL, NULL, NULL, NULL)";
			if($result = $mysqli->selectQuery($pwrQuery)) {
				return true;
			}
		...
		// only return false if the above two INSERTS fail
		return false;
...
//ChromePhp::log($query);
			...
			//todo: make a function so I only have to do this once 
